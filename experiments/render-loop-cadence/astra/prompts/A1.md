...
/tmp/weaver-exp-astra/runs/A1  (create it with `npx --no-install weaver init /tmp/weaver-exp-astra/runs/A1`; it will not exist yet)
...
- `npx --no-install weaver check /tmp/weaver-exp-astra/runs/A1` is the authority for statically knowable widget errors. It must pass before you finish.
...
Fixed clock for every capture: 2026-09-04T09:41:00.000Z (a Friday). Copy /tmp/weaver-exp-astra/clicks.actions to /tmp/weaver-exp-astra/runs/A1/clicks.actions first. It clicks "Log session" three times; the expected result is Friday showing 3, every other day 0, label "3 / 20", bar at 15%.

```
mkdir -p /tmp/weaver-exp-astra/runs/A1/captures
npx --no-install weaver capture /tmp/weaver-exp-astra/runs/A1 --clock 2026-09-04T09:41:00.000Z --out /tmp/weaver-exp-astra/runs/A1/captures/NN-initial.png
npx --no-install weaver capture /tmp/weaver-exp-astra/runs/A1 --clock 2026-09-04T09:41:00.000Z --action-file /tmp/weaver-exp-astra/runs/A1/clicks.actions --out /tmp/weaver-exp-astra/runs/A1/captures/NN-after-clicks.png
```
...
## When you are done
Write /tmp/weaver-exp-astra/runs/A1/REPORT.md containing:
- Condition letter.
...
## Your condition: A (check-only)
You may run `weaver check` as often as you like. You must NEVER run `weaver capture`, and you never see pixels or a snapshot. Build from the contract, the examples and your own reasoning. Stop when you believe the widget matches the spec. Leave /tmp/weaver-exp-astra/runs/A1/captures absent.
