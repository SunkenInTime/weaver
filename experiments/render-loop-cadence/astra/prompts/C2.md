...
/tmp/weaver-exp-astra/runs/C2  (create it with `npx --no-install weaver init /tmp/weaver-exp-astra/runs/C2`; it will not exist yet)
...
- `npx --no-install weaver check /tmp/weaver-exp-astra/runs/C2` is the authority for statically knowable widget errors. It must pass before you finish.
...
Fixed clock for every capture: 2026-09-04T09:41:00.000Z (a Friday). Copy /tmp/weaver-exp-astra/clicks.actions to /tmp/weaver-exp-astra/runs/C2/clicks.actions first. It clicks "Log session" three times; the expected result is Friday showing 3, every other day 0, label "3 / 20", bar at 15%.

```
mkdir -p /tmp/weaver-exp-astra/runs/C2/captures
npx --no-install weaver capture /tmp/weaver-exp-astra/runs/C2 --clock 2026-09-04T09:41:00.000Z --out /tmp/weaver-exp-astra/runs/C2/captures/NN-initial.png
npx --no-install weaver capture /tmp/weaver-exp-astra/runs/C2 --clock 2026-09-04T09:41:00.000Z --action-file /tmp/weaver-exp-astra/runs/C2/clicks.actions --out /tmp/weaver-exp-astra/runs/C2/captures/NN-after-clicks.png
```
...
## When you are done
Write /tmp/weaver-exp-astra/runs/C2/REPORT.md containing:
- Condition letter.
...
## Your condition: C (progressive capture)
Build in four visual slices and run the Render loop (check, capture, open the PNG, read the snapshot and receipt) at the end of each slice before starting the next:
1. Frame and header row only (capture initial state).
2. Add the seven day cells (capture initial state).
3. Add the progress bar (capture initial state).
4. Add the buttons and storage (capture initial state AND after-clicks state).
Fix what you see at each slice before moving on. You may re-capture within a slice as often as you like. After slice 4, keep iterating until you are satisfied.
