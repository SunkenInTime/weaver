...
| Entitlement behavior at the macOS 15.4 floor | **PASS** | The technique introduced for the 15.4 gate still works on the installed macOS 26.5.1. `adapter ... test` exited 0. `codesign` exposed no explicit entitlement dictionary on `/usr/bin/perl`; the observable qualifying identity is its Apple platform signature and `com.apple.perl` identifier. Exact 15.4 execution was not repeated on this current-OS machine. |
